...
import {
  ...
  XCircle, Clock, Eye, Calendar, Phone, Mail
} from 'lucide-react';
...
export default function RequestsPage() {
  ...
  const [selectedRequest, setSelectedRequest] = useState<typeof mockSolicitudes[0] | null>(null);
  ...
  const pendientes = mockSolicitudes.filter(r => r.estado === 'pendiente');
  const aprobadas = mockSolicitudes.filter(r => r.estado === 'aprobada');
  const rechazadas = mockSolicitudes.filter(r => r.estado === 'rechazada');
  ...
  const RequestCard = ({ request }: { request: typeof mockSolicitudes[0] }) => (
